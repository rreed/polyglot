...
Usage: go install ./11.go; ./11 (may require some GOPATH export fiddling, have not tested widely)
*/
...
func main() {
    ...
    for i = 3; i < 20; i++ {
        for j = 0; j < 16; j++ {
            otherDiagonalProduct := GRID[i][j] * GRID[i-1][j+1] * GRID[i-2][j+2] * GRID[i-3][j+3]
            if otherDiagonalProduct > maxProduct {
                maxProduct = otherDiagonalProduct
            }
        }
    }
